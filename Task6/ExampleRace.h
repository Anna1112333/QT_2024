...
//Класс-обертка QThread, который будет распределять наш класс по
//потокам. Именно так правильно использовать QThread,
//можно переопределить класс run, но разработчики Qt рекомендуют
//делать именно так.
...
public:
    Controller(QMutex* m) : mut(m) {
        ...
        connect(&workerThread, &QThread::finished, race,
                &QObject::deleteLater); //По завершению потока удаляем экземпляр класса
        connect(this, &Controller::operate, race,
                &ExampleRace::DoWork); //Запускаем "тяжелый" метод
        connect(race, &ExampleRace::sig_Finish, this,
                &Controller::sig_WorkFinish); //Пробрасываем сигнал для отображения в GUI
